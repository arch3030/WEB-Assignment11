...
{
    var readdata = await readJson('postDB.json');
    res.end(readdata);
})
...
{
    var readdata = await readJson('postDB.json');
    var writedata = writeJson(readdata,req.body,'postDB.json');
...
app.post('/checkLogin',async (req,res) => 
{
    ...
    var readdata = await readJson('userDB.json');
...
{
    var data = await readJson("userDB.json.JSON");
    return new Promise((resolve, rejects) => {
    ...
    for(var data_new = 0 ; data_new<key.length;data_new++)
    {
        if(username == data_origin[key[data_new]].username)//เช็คว่าตัวชื่อผู้ใช้ตรงกันหรือเปล่า
        {
            ...
            fs.writeFile("userDB.JSON",data_new,(err) => {
                if (err) 
                    reject(err);
                else
                {
